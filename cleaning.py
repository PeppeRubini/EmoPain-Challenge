...
def clean(label_path, geometric_path):
    for file in os.listdir(label_path):
        if (file.startswith("P")):
            ...
            df_label = df_merged.pop('label')
            df_label.to_csv(str(label_path).replace("Dataset", "CleanDataset") + file, index=False, header=None)
            # salvo il file delle feature
            df_geo = df_merged[header_list]
            df_geo.to_csv(str(geometric_path).replace("Dataset", "CleanDataset") + file, index=False)
